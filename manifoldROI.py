...
parser.add_argument('--eigs', help="specify which eigenvectors to plot (default=0,1)", default='0,1', dest="eigs",
                    type=coords, nargs=1)
parser.add_argument("--override",
                    help="override the existing hdf5. WARNING: don't use this if you don't know what you are doing",
                    action='store_true')
...
class imageRetriever:
    def __init__(self, filepath, expName, runs, tag, dataset, detInfo):
        ...
        print
        "totalHits: ", self.totalHits, self.allHitInd
    ...
    def getAssemImage(self, globalIndex):
        ...
        print
        "#### run number, event: ", int(self.runList[globalIndex]), int(self.eventInd[globalIndex])
...
if __name__ == '__main__':
    ...
    if args.keepROI:
        print
        "Keep points inside ROI"
    else:
        print
        "Throw away points inside ROI"
    plt.show()
    ...
    for i in np.unique(myFileInd):
        ...
        print
        "New filename, numHits: ", newFilename, newNumHits
